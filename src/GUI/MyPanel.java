...
public class MyPanel extends javax.swing.JFrame {
	...
	private void initComponents() {
		...
		jPanel1 = new PanelComp(this);        //create JPanel inside JFrame  
	...
	private void jPanel1MouseClicked(java.awt.event.MouseEvent evt) {                                     
		...
		// if press before on add my player button 
		if(myplayer_addable)
		{
	...
	/**
	 * load the game and present it to user , the method add all needs components and paint to screen
	 */
	...
	/**
	 * Regular running game, start play the game with the mouse clicks
	 */
	public void runGame() {
		runTheGame = true;
	...
	/**
	 * Auto Running game the game running by specifics graghics algorithms that get the route to the player and moving him auto. 
	 */
	public void runGameAuto () {
		GraphicGame alg = new GraphicGame(game);   //create graph to the game 
		
		Point3D gpsPoint = alg.startingPoint();    //start point of player
		...
		Thread th = new Thread(new Runnable() {
			public void run() {
				...
				while(play1.isRuning()) {
					...
					String outOfb = stati[5];
					int ch2 = outOfb.indexOf(':');
					String sr2 = outOfb.substring(ch2+1);
					int outOfBox = Integer.parseInt(sr2);

					if(game.IsCloseGhots() && outOfBox == last_outOfBox) {// if ghost in radius - runaway
						goThere =alg.runAway(goThere, azimuth);
						play1.rotate(goThere);
						last_outOfBox = outOfBox;
					}
					else
						if(!(path=alg.update(game)).isEmpty()){// calculate the path that the player should to go
						target = path.get(0);
	...
	class PacmanLabel extends JLabel{
		...
		public void update( int width, int height) {
			...
			if (this.packmen.isEaten()==true) {//if pacman eaten dont show him in the game
				isEaten=true;
			}
	...
	/**
	 * This class represent the ghost JLable that it according the ghost from the game on screen
	 * @author Netanel
	 * @author Carmel
	 *
	 */
	...
	/**
	 * This class represnt the player JLable from the game on screen
	 * @author Netanel
	 * @author Carmel
	 *
	 */
	class MyPlayerLabel extends JLabel{
		...
		/**
		 * init first location of the player and preper variable to paint
		 * @param width
		 * @param height
		 * @param lat
		 * @param lon
	...
	/**
	 * This class represent the box from the game
	 * @author Netanel
	 * @author Carmel
	 *
	 */
	class BoxLabel extends JLabel{
		...
		/**
		 * init first location of the Box and preper variable to paint
		 * @param width
		 * @param height
		 * @param lat
		 * @param lon
		...
		/**
		 * if paint is playing update the box according to size of screen 
		 * @param width
		 * @param height
		 */
		public void update(int width, int height) {
